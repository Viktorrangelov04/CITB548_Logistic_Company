...
const generateAccessToken = (entity, type) => {
  return jwt.sign({ id: entity._id, type: type }, JWT_SECRET, {
    expiresIn: "15m",
  });
};

const generateRefreshToken = (entity, type) => {
  return jwt.sign({ id: entity._id, type: type }, REFRESH_SECRET, {
    expiresIn: "7d",
  });
};
...
export const getMe = async (req, res) => {
  try {
    let entity;
    if (req.entity.type === "client") {
      entity = await User.findById(req.entity.id).select("-password");
    } else if (req.entity.type === "employee-office") {
      entity = await User.findById(req.entity.id).select("-password");
    } else if (req.entity.type === "employee-courier") {
      entity = await User.findById(req.entity.id).select("-password");
...
export const login = async (req, res) => {
  try {
    ...
    if (!user)
      return res
        .status(401)
        .json({ message: "Account with that email doesn't exist" });
...
export const loginCompany = async (req, res) => {
  try {
    ...
    if (!company)
      return res
        .status(401)
        .json({ message: "Company with that email doesn't exist" });
